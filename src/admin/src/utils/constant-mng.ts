...
type IData = Record<GroupName, IItem[]>
...
// 所有常量分组名称
const groupNames = tupleStr('gender', 'role', 'payStatus', 'platform', 'productLine', 'client')

export type GroupName = typeof groupNames[number]

/* 系统常量管理 */
export class ConstantMng {
	private data: IData = {} as any
	...
	public formatGroupByFunction = <F extends (item: IItem) => any>(
		groupName: GroupName,
		formatter: F
	): ReturnType<F>[] => {
		const groupList = this.getGroup(groupName)
		return groupList.map(formatter)
	}
}
